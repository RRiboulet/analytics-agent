...
* M7.3 — synthesis: report generated by the agent (returned by `run()` and
  ...
  `groundedness_violation` (1e-6 relative tolerance reused from the M6
  ...
  Graph: new `synthesize` node after sub-analyses (routed only when the run
  completed); success stores the report and completes; a groundedness
  violation fails the run without storing the report (no retry —
  deterministic output reproduces the violation); synthesis `LLMError`
  reuses the bounded retry, routed to synthesis by evidence presence (the
  attempt budget is shared with decompose, keeping a single hard bound).
  `ManagerState` gained `groundedness_error`. Tests:
  `tests/test_manager_synthesize.py` (22 tests incl. graph integration);
  `app/manager` at 100% line + branch coverage.
